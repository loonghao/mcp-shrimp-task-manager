...
const globalAnalysisResultElement = document.getElementById('global-analysis-result'); // 假設 HTML 中有這個元素
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 新增：語言切換器事件監聽
  if (langSwitcher) {
    langSwitcher.addEventListener('change', (e) => changeLanguage(e.target.value));
  }
    ...
    .then(() => {
      ...
      if (selectedTaskId) {
        ...
        if (task) {
          ...
          // 如果選中的任務已不存在，清除詳情
          taskDetailsContent.innerHTML = `<p class="placeholder">${translate('task_details_placeholder')}</p>`;
          selectedTaskId = null;
          highlightNode(null);
        }
      } else {
...
async function fetchTasks() {
  try {
    ...
    if (tasks.length === 0) {
      taskListElement.innerHTML = `<div class="loading">${translate('task_list_loading')}</div>`;
    }
    ...
    if (tasksChanged) {
      ...
      console.log('No significant task changes detected, skipping full re-render.');
    ...
    // 避免覆蓋現有列表，除非是初始載入失敗
    if (tasks.length === 0) {
      taskListElement.innerHTML = `<div class="error">${translate('error_loading_tasks', {
        message: error.message,
      })}</div>`;
      if (progressIndicator) progressIndicator.style.display = 'none';
      if (dependencyGraphElement)
        dependencyGraphElement.innerHTML = `<div class="error">${translate('error_loading_graph')}</div>`;
    } else {
      showTemporaryError(translate('error_updating_tasks', { message: error.message }));
    }
  }
}
...
function compareDependencies(deps1, deps2) {
  ...
  const ids1 = new Set(arr1.map((dep) => (typeof dep === 'object' && dep !== null ? dep.taskId : dep)));
  const ids2 = new Set(arr2.map((dep) => (typeof dep === 'object' && dep !== null ? dep.taskId : dep)));
...
function renderTasks() {
  ...
  if (selectedTaskId) {
    ...
    if (taskExists) {
      const selectedElement = document.querySelector(`.task-item[data-id="${selectedTaskId}"]`);
      if (selectedElement) {
        selectedElement.classList.add('selected');
      }
    } else {
      // 如果選中的任務在新的列表中不存在了，清除選擇
      console.log(`Selected task ${selectedTaskId} no longer exists, clearing selection.`);
      selectedTaskId = null;
      taskDetailsContent.innerHTML = `<p class="placeholder">${translate('task_details_placeholder')}</p>`;
...
function selectTask(taskId) {
  // 清除旧的选中状态和高亮
  if (selectedTaskId) {
    const previousElement = document.querySelector(`.task-item[data-id="${selectedTaskId}"]`);
    if (previousElement) {
...
=======
  // 添加新的選中狀態
  const selectedElement = document.querySelector(`.task-item[data-id="${taskId}"]`);
>>>>>>> f7dac28 (refactor: improve code quality and ensure formatting standards (#4))
  if (selectedElement) {
    selectedElement.classList.add('selected');
  }
  ...
  if (!task) {
    taskDetailsContent.innerHTML = `<div class="placeholder">${translate('error_task_not_found')}</div>`;
    return;
  }
  ...
  taskDetailsContent.innerHTML = `
    <div class="task-details-header">
      ...
      <div class="task-meta">
        <span>${translate('task_detail_status_label')} <span id="detail-status" class="task-status"></span></span>
      </div>
    </div>
  ...
  const detailImplementationGuide = document.getElementById('detail-implementation-guide');
  const detailVerificationCriteria = document.getElementById('detail-verification-criteria');
  // 新增：獲取 Summary 相關元素
  ...
  if (detailStatus) {
    ...
    detailStatus.className = `task-status status-${task.status.replace('_', '-')}`;
  }
  if (detailDescription) detailDescription.textContent = task.description || translate('task_detail_no_description');
  if (detailImplementationGuide)
    detailImplementationGuide.textContent =
      task.implementationGuide || translate('task_detail_no_implementation_guide');
  if (detailVerificationCriteria)
    detailVerificationCriteria.textContent =
      task.verificationCriteria || translate('task_detail_no_verification_criteria');
  ...
  if (detailNotes) detailNotes.textContent = task.notes || translate('task_detail_no_notes');
  ...
  if (detailDependencies) {
    const dependenciesHtml =
      task.dependencies && task.dependencies.length
        ? task.dependencies
            .map((dep) => {
              const depId = typeof dep === 'object' && dep !== null && dep.taskId ? dep.taskId : dep;
              const depTask = tasks.find((t) => t.id === depId);
              // Translate the fallback text for unknown dependency
              const depName = depTask ? depTask.name : `${translate('task_detail_unknown_dependency')}(${depId})`;
            ...
            .join('')
        : `<span class="placeholder">${translate('task_detail_no_dependencies')}</span>`; // Translate placeholder
    ...
    const relatedFilesHtml =
      task.relatedFiles && task.relatedFiles.length
        ? task.relatedFiles
            ...
            .join('')
        : `<span class="placeholder">${translate('task_detail_no_related_files')}</span>`; // Translate placeholder
...
>>>>>>> f7dac28 (refactor: improve code quality and ensure formatting standards (#4))
  ...
  const linkSelection = g
    ...
    .selectAll('line.link')
    .data(links, (d) => `${d.source.id || d.source}-${d.target.id || d.target}`); // Key function 基於 source/target ID
...
=======
  // Exit - 移除舊連結
  linkSelection.exit().transition('exit').duration(300).attr('stroke-opacity', 0).remove();
>>>>>>> f7dac28 (refactor: improve code quality and ensure formatting standards (#4))
...
=======
  ...
  linkUpdate.transition('update').duration(500).attr('stroke-opacity', 0.6).attr('stroke-width', 1.5);
>>>>>>> f7dac28 (refactor: improve code quality and ensure formatting standards (#4))
...
=======
    ...
    .attr('transform', (d) => `translate(${d.x || Math.random() * width}, ${d.y || Math.random() * height}) scale(0)`)
    .attr('opacity', 0)
    .call(drag(simulation)); // 添加拖拽
...
>>>>>>> f7dac28 (refactor: improve code quality and ensure formatting standards (#4))
  ...
  nodeEnter.append('title').text((d) => `${d.name} (${getStatusText(d.status)})`);
...
=======
  ...
  // 更新節點顏色 (單獨過渡)
  nodeUpdate.select('circle').transition('color').duration(500).attr('fill', getNodeColor); // 使用已有的 getNodeColor 函數

  // 更新節點狀態 Class (即時更新，無需過渡)
  nodeUpdate.attr('class', (d) => `node-item status-${getStatusClass(d.status)}`);
>>>>>>> f7dac28 (refactor: improve code quality and ensure formatting standards (#4))
...
function updateProgressIndicator() {
  ...
  const completedTasks = tasks.filter((task) => task.status === 'completed' || task.status === '已完成').length;
  const inProgressTasks = tasks.filter((task) => task.status === 'in_progress' || task.status === '進行中').length;
  const pendingTasks = tasks.filter((task) => task.status === 'pending' || task.status === '待處理').length;
  ...
  progressLabels.innerHTML = `
    ...
    <span class="label-pending">${translate('progress_pending')}: ${pendingTasks} (${pendingPercent.toFixed(1)}%)</span>
    <span class="label-total">${translate('progress_total')}: ${totalTasks}</span>
  `;
}
...
function renderGlobalAnalysisResult() {
  ...
  if (globalAnalysisResult) {
    targetElement.innerHTML = `
            <h4 data-i18n-key="global_analysis_title">${translate('global_analysis_title')}</h4> 
