 ...
 * @returns 生成的 prompt
 */
export function getResearchModePrompt(params: ResearchModePromptParams): string {
  // 處理之前的研究狀態
  let previousStateContent = '';
  if (params.previousState && params.previousState.trim() !== '') {
    const previousStateTemplate = loadPromptFromTemplate('researchMode/previousState.md');
