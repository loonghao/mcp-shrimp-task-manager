...
export async function queryTask({ query, isId = false, page = 1, pageSize = 3 }: z.infer<typeof queryTaskSchema>) {
  try {
    // 使用系統指令搜尋函數
    ...
    return {
      content: [
        {
          type: 'text' as const,
          text: `## 系統錯誤\n\n查詢任務時發生錯誤: ${error instanceof Error ? error.message : String(error)}`,
        },
      ],
      isError: true,
    };
