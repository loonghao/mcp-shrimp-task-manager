...
async function analyzeTeamPerformance(
  ...
  // 获取协作模式
  const allRoles = args.targetRoles as TeamRole[] || [
    'product-manager', 'ui-designer', 'ux-designer', 'frontend-developer',
  ...
  const avgCollaborationSuccess = collaborationPatterns.length > 0
  ...
  return {
    ...
    trends: {
      recentActivity: learningRecords.filter(r => 
...
async function analyzeCollaborationEffectiveness(
  ...
  // 协作频率分析
  const collaborationFrequency = patterns.map(p => ({
    name: p.name,
...
// 分析角色贡献
async function analyzeRoleContribution(
  teamMemoryManager: TeamMemoryManager,
  args: AnalyzeTeamCollaborationInput
) {
  console.log("👥 分析角色贡献...");
  ...
  // 按角色统计贡献
  const roleContributions = new Map<string, {
  ...
  const roleAnalysis = Array.from(roleContributions.entries()).map(([role, contrib]) => ({
    role,
...
function calculateCollaborationTrend(patterns: any[]): string {
  const recentPatterns = patterns.filter(p => 
    new Date(p.lastUsed).getTime() > Date.now() - 30 * 24 * 60 * 60 * 1000
  );
