...
const AdjustTasksFromContextSchema = z.object({
  ...
  adjustmentScope: z.enum(["all", "pending", "related"]).default("pending").describe("调整范围：all(所有任务)、pending(待执行任务)、related(相关任务)"),
  ...
  contextFactors: z.array(z.string()).optional().describe("特定的上下文因素，如['performance', 'security', 'timeline']")
});
...
export async function adjustTasksFromContext(args: AdjustTasksFromContextInput) {
  try {
    ...
    console.log(`🔍 开始分析项目 "${projectContext.projectName}" 的执行上下文...`);
    ...
    console.log(`🔬 找到 ${executionContexts.length} 个执行上下文进行分析`);
    ...
    for (const context of executionContexts) {
      console.log(`📊 分析执行上下文: ${context.taskId} (${context.executionId})`);
    ...
    console.log('');
    console.log('📈 分析结果统计:');
    console.log(`- 分析的执行上下文: ${executionContexts.length}个`);
    console.log(`- 目标任务数: ${targetTasks.length}个`);
    console.log(`- 生成建议数: ${uniqueSuggestions.length}个`);
    console.log(`- 高置信度建议: ${uniqueSuggestions.filter(s => s.confidence > 0.7).length}个`);
    
    if (autoApply) {
      console.log(`- 自动应用调整: ${appliedAdjustments.length}个`);
    }
    ...
    if (uniqueSuggestions.length > 0) {
      ...
      uniqueSuggestions
        ...
        .forEach((suggestion, index) => {
          console.log(`${index + 1}. [${suggestion.adjustmentType}] ${suggestion.reasoning} (置信度: ${Math.round(suggestion.confidence * 100)}%)`);
        });
    }
    ...
    if (riskAnalysis && riskAnalysis.risks.length > 0) {
      console.log('');
      console.log('⚠️ 风险提醒:');
      riskAnalysis.risks.slice(0, 3).forEach((risk, index) => {
    ...
    console.error("❌ 基于上下文的任务调整失败:", error);
    return {
      success: false,
...
async function generateRiskAnalysis(suggestions: any[], tasks: any[]) {
  ...
  // 分析高风险调整
  const highRiskSuggestions = suggestions.filter(s => 
    s.impact.riskChange === 'increased' || s.confidence < 0.5
  );
