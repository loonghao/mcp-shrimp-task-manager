...
- **📁 文檔路徑管理**：結構化文檔組織，為分析、反思、研究和團隊協作文檔提供專案特定路徑
...
蝦米任務管理器提供結構化的文檔組織，具有專案特定路徑。所有 AI 生成的文檔（分析、反思、研究、團隊協作）都會自動存儲在有組織的目錄中：

```
$DATA_DIR/projects/$PROJECT_NAME/docs/
├── project/          # 核心專案文檔（README、架構、路線圖）
├── prd/              # 產品需求和里程碑
├── tasks/            # 任務相關文檔
│   ├── completed/    # 已完成任務總結
│   ├── in-progress/  # 進行中任務分析
│   ├── pending/      # 待執行任務規劃
│   └── by-role/      # 按角色分類（前端、後端、測試、運維、設計）
├── analysis/         # 分析報告（複雜度、風險、性能、安全）
├── reflection/       # 反思總結（衝刺回顧、經驗教訓、改進建議）
├── research/         # 研究文檔（技術調研、競品分析、最佳實踐）
├── team/             # 團隊協作文檔（知識庫、協作模式、角色職責）
└── templates/        # 文檔模板
```
...
- **自動組織**：文檔按類型和用途自動分類
- **專案隔離**：每個專案維護自己的文檔結構
- **團隊協作**：為團隊知識分享和協作提供結構化路徑
- **易於導航**：清晰的層次結構使查找文檔變得直觀

**使用方式**：系統在創建文檔時會自動使用正確的路徑。您也可以使用 `get_documentation_path` 工具明確請求文檔路徑。
...
覆蓋率報告會自動發布為 PR 評論，如果覆蓋率低於 50% 閾值，建置將失敗。
...
| **📁 文檔管理** | `get_documentation_path` | 獲取專案特定的文檔存儲路徑 |
